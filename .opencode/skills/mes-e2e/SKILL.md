...
**In CI the stack is already running** (started with `nohup` before the agent
invocation). Do NOT start or stop it — just navigate and test. Backend logs
are at `/tmp/backend.log` and `/tmp/frontend.log`.
