...
{
    public async Task<PagedResponse<ShiftResponse>> Handle(
        BrowseShiftsRequest request, CancellationToken cancellationToken)
    {
    ...
    internal static ShiftResponse Map(Shift s) =>
        new(s.Id, s.Code, s.Name, s.Description, s.StartTime, s.EndTime, s.IsActive);
}
