...
internal sealed class GetProductByScanRequestHandler(IProductsRepository productsRepository)
    : IRequestHandler<GetProductByScanRequest, ProductResponse>
{
    public async Task<ProductResponse> Handle(GetProductByScanRequest request, CancellationToken cancellationToken)
    {
        var value = request.Value?.Trim();

        if (string.IsNullOrWhiteSpace(value))
            throw new ValidationException(nameof(request.Value), "Scan value is required.");

        var product = await productsRepository.GetByScanAsync(value, cancellationToken);

        if (product is null)
            throw new NotFoundException("Product", value);
