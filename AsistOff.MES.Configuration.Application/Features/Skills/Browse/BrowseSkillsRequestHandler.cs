...
internal sealed class BrowseSkillsRequestHandler(ISkillsRepository repository)
    : IRequestHandler<BrowseSkillsRequest, PagedResponse<SkillResponse>>
{
    public async Task<PagedResponse<SkillResponse>> Handle(BrowseSkillsRequest request, CancellationToken cancellationToken)
    {
        ...
        var paginator = new Paginator<Skill>(predicate, request);
        var items = await repository.BrowseAsync(paginator, cancellationToken);

        var result = items.Select(Map).ToList();
        return new PagedSkillsResponse(result, totalCount, request.PageSize);
    }

    internal static SkillResponse Map(Skill s) => new(s.Id, s.Code, s.Name, s.Description, s.IsActive);
}
